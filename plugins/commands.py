...
from info import CHANNELS, FSUB_PICS, ADMINS,  LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, REQST_CHANNEL, GRP_LNK, SUPPORT_CHAT_ID, MAX_B_TN, VERIFY, REACTIONS, HOW_TO_VERIFY, PICS, FTMBOTZX_VERIFIED_LOG, SUBSCRIPTION, FTMBOTZX_MOVIE_UPDATE_CHANNEL_LNK, STREAM_MODE, EMOJI_MODE, OWNER_LNK, OWNER_UPI_ID, QR_CODE
...
async def cleanup_invalid_media(file_id):
    """Remove invalid media from database"""
    try:
        # Remove from both databases
        result1 = await Media.collection.delete_many({'file_id': file_id})
        result2 = await Media2.collection.delete_many({'file_id': file_id})
        
        deleted_count = result1.deleted_count + result2.deleted_count
        if deleted_count > 0:
            logger.info(f"Removed {deleted_count} invalid media entries for file_id: {file_id}")
        return deleted_count
    except Exception as e:
        logger.error(f"Error cleaning up invalid media {file_id}: {e}")
        return 0
...
async def start(client, message):
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            if STREAM_MODE:
                btn = [
                    ...
                    [InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=FTMBOTZX_MOVIE_UPDATE_CHANNEL_LNK)]  # Keep this line unchanged
                ]
            else:
                btn = [
                    [InlineKeyboardButton('📌 ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇꜱ ᴄʜᴀɴɴᴇʟ 📌', url=FTMBOTZX_MOVIE_UPDATE_CHANNEL_LNK)]
                ]
            try:
                ...
                try:
                    ...
                    if "MEDIA_EMPTY" in str(retry_e) or "FILE_REFERENCE_EXPIRED" in str(retry_e):
                        logger.error(f"Media empty error for batch file_id {msg.get('file_id')}: {retry_e}")
                    else:
                        logger.error(f"Error sending batch media after retry: {retry_e}")
                    continue
            except Exception as e:
                if "MEDIA_EMPTY" in str(e) or "FILE_REFERENCE_EXPIRED" in str(e):
                    logger.error(f"Media empty error for batch file_id {msg.get('file_id')}: {e}")
                else:
        ...
        if is_valid == True:
            ...
            await client.send_message(chat_id=FTMBOTZX_VERIFIED_LOG, text=ftmbotzx_message)
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            try:
                ...
                if "MEDIA_EMPTY" in str(e) or "FILE_REFERENCE_EXPIRED" in str(e):
                    logger.error(f"Media empty error for allfiles file_id {file_id}: {e}")
                    continue  # Skip this file and continue with others
                else:
                    logger.error(f"Error sending allfiles media: {e}")
                    continue
            filesarr.append(msg)
    ...
    if not files_:
        ...
        try:
            ...
            try:
                ...
                if "MEDIA_EMPTY" in str(e) or "FILE_REFERENCE_EXPIRED" in str(e):
                    await message.reply_text("⚠️ This file is no longer available. The file may have been deleted or expired from Telegram's servers.")
                    logger.error(f"Media empty error for file_id {file_id}: {e}")
                else:
                    await message.reply_text("❌ An error occurred while sending the file. Please try again later.")
