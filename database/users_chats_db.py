...
from info import DATABASE_NAME, DATABASE_URI, DATABASE_URI2, IMDB, IMDB_TEMPLATE, MELCOW_NEW_USERS, P_TTI_SHOW_OFF, SINGLE_BUTTON, SPELL_CHECK_REPLY, PROTECT_CONTENT, AUTO_DELETE, MAX_BTN, AUTO_FFILTER, SHORTLINK_API, SHORTLINK_URL, IS_SHORTLINK, TUTORIAL, IS_TUTORIAL, VERIFY, PM_SEARCH, MULTI_FSUB, FTMBOTZX_MOVIE_UPDATE_NOTIFICATION, LOG_CHANNEL
import datetime
import pytz  
...
class Database:
    ...
    async def movie_update_status(self, bot_id):
        bot = await self.botcol.find_one({'id': int(bot_id)}, {'FTMBOTZX_MOVIE_UPDATE_NOTIFICATION': 1})
        return bot.get('FTMBOTZX_MOVIE_UPDATE_NOTIFICATION', FTMBOTZX_MOVIE_UPDATE_NOTIFICATION) if bot else FTMBOTZX_MOVIE_UPDATE_NOTIFICATION   

    async def update_movie_update_status(self, bot_id, enable):
            await self.botcol.update_one(
                {'id': int(bot_id)},
                {'$set': {'FTMBOTZX_MOVIE_UPDATE_NOTIFICATION': enable}},
                upsert=True
            )
